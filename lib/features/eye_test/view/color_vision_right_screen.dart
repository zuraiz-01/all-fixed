...
import 'package:eye_buddy/features/eye_test/view/color_vision_result_screen.dart';
...
class ColorVisionRightScreen extends StatefulWidget {
  const ColorVisionRightScreen({super.key});

  @override
  State<ColorVisionRightScreen> createState() => _ColorVisionRightScreenState();
}

class _ColorVisionRightScreenState extends State<ColorVisionRightScreen> {
  ...
  void _onConfirm(EyeTestController controller) {
    setState(() {
      ...
      if (_text.text.trim() == _answers[answerIndex]) {
        controller.incrementColorVisionRight();
      }
      ...
      if (_tap >= 6) {
        Get.to(() => const ColorVisionResultScreen());
      }
    });
  }
